...
unordered_map<string, string> usercookies; //session本地缓存  加入url-utf8转存机制  cookie存url编码， 用户名用utf-8编码支持中文, 数据库中也存url编码

void http_conn::clear_cookie() {
    if (usercookies.size() > 2000) usercookies.clear();
}
...
http_conn::HTTP_CODE http_conn::parse_headers(char *text)
{
    ...
    else if (strncasecmp(text, "Cookie:", 7) == 0)
    {
        ...
        if (usercookies.count(now_cookie)) {
            if (level1_users.count(now_cookie)) m_level = VIP;
            else if (level2_users.count(now_cookie)) m_level = SVIP;
            else m_level = COMMON;
        }
    }
    else  //其它头部数据按需处理
...
http_conn::HTTP_CODE http_conn::do_request()
{
    ...
    if ((url == "2CGISQL.cgi" || url == "3CGISQL.cgi") && m_method == POST)
    {
        ...
        else if (url == "2CGISQL.cgi")
        {
            if (users.find(name) != users.end() && users[name] == password) {
                ...
                usercookies[name] = UrlDecode(name); //存储cookie
                //登陆成功后 需要更新用户权限
                if (level1_users.count(now_cookie)) m_level = VIP;
                else if (level2_users.count(now_cookie)) m_level = SVIP;
                else m_level = COMMON;
    ...
    if (cgi == 1) {
        ...
        if (m_passwd == FILE_LOAD_PASSWD) {
            ...
            if (!ret) {
                level1_users.insert(now_cookie);
                m_level = VIP;
    ...
    if (final_url.find(".html") != -1) {
        ...
        string content1( (istreambuf_iterator<char>(ifs1) ),
                        (istreambuf_iterator<char>() ) );
        ifs1.close();
        if (final_url.find("picture.html") != -1) tag_judge = 1;
        if (tag_judge == 1) { //作品上传
            //插入用户显示区
            string final1;
            string final2;
            if (m_level == LOGOUT) {}//未登陆状态， 
                ...
                final1 = "<li><a class='smoothScroll'; style='color:#ff0000'><font size='3' >VIP用户: "
                + usercookies[now_cookie] + "</font><br/></a></li>\n" + 
                ...
                final1 = "<li><a class='smoothScroll'; style='color:#ff0000'><font size='3' >SVIP用户: "
                + usercookies[now_cookie] + "</font><br/></a></li>\n" + 
                "<li><a class='smoothScroll' href='/logout.html'; style='color:#000000'><font size='3'>用户注销</font><br/></a></li>";
            ...
            if (m_level == LOGOUT) { //未登陆状态
                ...
                final2 = "<li><a class='smoothScroll' href='/picture.html'><font size='6'>字画集</font><br/></a></li>\n";
            }
            else if (m_level == SVIP) { //SVIP
                final1 = "<li><a class='smoothScroll'; style='color:#ff0000'><font size='3' >SVIP用户: "
                + usercookies[now_cookie] + "</font><br/></a></li>\n" + 
...
{
    return add_content_length(content_len) && add_linger() && (set_cookie.empty() || add_cookie(set_cookie.c_str())) &&
           add_blank_line();
}
...
{
    return add_response("Set-Cookie:userid=%s; max-age=7200; path=/\r\n", username);
}
bool http_conn::process_write(HTTP_CODE ret)
{
    ...
    case FILE_REQUEST:
    {
        ...
        if (m_file_stat.st_size != 0)
        {
            ...
            // if (!set_cookie.empty()) add_cookie(set_cookie.c_str());  放add_headers中
